...
package com.florent.location.ui.lease.components
        ...
        SectionCard {
            ...
            if (
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(UiTokens.SpacingS)) {
                    housing.mailboxLabel?.let { NonInteractiveChip(label = "Boîte $it", icon = Icons.Outlined.Inbox) }
                    housing.meterGasId?.let { NonInteractiveChip(label = "Gaz $it", icon = Icons.Outlined.Payments) }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(UiTokens.SpacingS)) {
                    housing.meterElectricityId?.let { NonInteractiveChip(label = "Élec. $it", icon = Icons.Outlined.Payments) }
                    housing.meterWaterId?.let { NonInteractiveChip(label = "Eau $it", icon = Icons.Outlined.Payments) }
                }
            }
        }
    }
        ...
        Column(modifier = Modifier.padding(UiTokens.SpacingL), verticalArrangement = Arrangement.spacedBy(UiTokens.SpacingM)) {
            ...
            OutlinedCard(
                ...
                Column(modifier = Modifier.padding(UiTokens.SpacingM), verticalArrangement = Arrangement.spacedBy(UiTokens.SpacingS)) {
                    ...
                    LabeledValueRow(label = "Échéance", value = "Jour ${lease.rentDueDayOfMonth}")
                    LabeledValueRow(label = "Début", value = formatEpochDay(lease.startDateEpochDay))
                }
            }
        }
    }
        ...
        SectionCard(tonalColor = MaterialTheme.colorScheme.surfaceContainerHigh) {
            state.indexationPolicy?.let { policy ->
                ...
                Row(
                    ...
                    Icon(imageVector = Icons.Outlined.EventAvailable, contentDescription = null, tint = MaterialTheme.colorScheme.primary)
                ...
                LabeledValueRow(label = "Anniversaire", value = formatEpochDay(policy.anniversaryEpochDay))
            }
        }
        SectionCard {
            ...
            if (state.indexationHistory.isEmpty()) {
                ...
                state.indexationHistory.forEach { event ->
                    TimelineListItem(
                        ...
                        subtitle = "${formatCurrency(event.baseRentCents)} → ${formatCurrency(event.newRentCents)}",
...
private fun KeysSection(
    ...
    Column(modifier = modifier, verticalArrangement = Arrangement.spacedBy(UiTokens.SpacingS)) {
        SectionHeader(title = "Clés", supportingText = "Suivi des clés remises.")
        if (keys.isEmpty()) {
            SectionCard {
                Text(text = "Aucune clé enregistrée.", style = MaterialTheme.typography.bodyMedium)
                Text(
                ...
                SectionCard {
                    Row(
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(UiTokens.SpacingXs)) {
                            Text(text = key.type, style = MaterialTheme.typography.titleMedium)
                            key.deviceLabel?.let { StatusBadge(text = it) }
                            LabeledValueRow(label = "Remise", value = formatEpochDay(key.handedOverEpochDay))
                        }
                        TextButton(onClick = { onDeleteKey(key.id) }, modifier = Modifier.focusable()) {
                            Text(text = "Supprimer")
                        }
                    }
                }
        ...
        PrimaryActionRow(primaryLabel = "Ajouter une clé", onPrimary = onAddKey)
    }
}

@Composable
private fun DestructiveActionsSection(isActive: Boolean, onCloseLease: () -> Unit, modifier: Modifier = Modifier) {
    if (!isActive) return
